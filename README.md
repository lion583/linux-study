...
2. I2C协议介绍--对于常见的通信协议还需要重新出一个汇总介绍详细通信过程以及如何查看波形这里只是暂时回顾i2c
   ...
   4. 硬件设计以及主从信号处理：   
   ![I2C硬件设计以及主从信号处理](./i2c/i2c主从信号处理1.png)
   ![I2C硬件设计以及主从信号处理](./i2c/i2c主从信号处理2.png)
3. SMBus
   1. SMBus是I2C的一个子集，定义了一些更严格的协议和电气特性，主要用于系统管理相关的设备通信。SMBus在I2C的基础上增加了一些功能，
   2. 和I2C的主要区别：
        1. 对于VDD规范不同：I2C的VDD范围更广，而SMBus通常工作在3.3V或5V
        2. 对于时钟线SCL频率需求不一样，I2C最大最小无限制，SMBus最大频率100kHz，最小频率10kHz
        3. 对于回应型号：I2C没有强制要求从设备必须回应，而SMBus要求从设备必须回应
        4. 对于发送数据逻辑：I2C完成一个周期读写后，还需要重新发送地址访问数据，SMBus可以访问同一个地址的数据时不需要重新发送地址，重复START即可
        5. SMBus有更复杂的协议支持，如块读写、命令代码等
